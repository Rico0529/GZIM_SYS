	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="办理地点" prop="banlididian" >
					<el-input v-model="ruleForm.banlididian" placeholder="办理地点" clearable  :readonly="ro.banlididian"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="办理地点" prop="banlididian" >
					<el-input v-model="ruleForm.banlididian" placeholder="办理地点" readonly></el-input>
				</el-form-item>
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='banlididian'){
							this.ruleForm.banlididian = obj[o];
							this.ro.banlididian = true;
							continue;
						}
						if(o=='yuyueshijian'){
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.yuyuexiangqing = this.ruleForm.yuyuexiangqing.replace(reg,'../../../GZIM_SYS/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `yewuyuyue/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.yewuyuyueCrossAddOrUpdateFlag = false;
											this.parent.search();
