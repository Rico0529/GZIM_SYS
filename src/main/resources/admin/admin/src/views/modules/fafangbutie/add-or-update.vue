	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="发放资金" prop="fafangzijin" >
					<el-input-number v-model="ruleForm.fafangzijin" placeholder="发放资金" :disabled="ro.fafangzijin"></el-input-number>
				</el-form-item>
				<el-form-item v-else class="input" label="发放资金" prop="fafangzijin" >
					<el-input v-model="ruleForm.fafangzijin" placeholder="发放资金" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="input" v-else-if="ruleForm.fafangshijian" label="发放时间" prop="fafangshijian" >
			...
			<el-form-item class="textarea" v-if="type!='info'" label="发放内容" prop="fafangneirong" >
				<el-input
			...
			<el-form-item v-else-if="ruleForm.fafangneirong" label="发放内容" prop="fafangneirong" >
				<span class="text">{{ruleForm.fafangneirong}}</span>
			</el-form-item>
			<el-form-item v-if="type!='info'"  label="发放详情" prop="fafangxiangqing" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
			...
			<el-form-item v-else-if="ruleForm.fafangxiangqing" label="发放详情" prop="fafangxiangqing" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.fafangxiangqing"></span>
			</el-form-item>
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.fafangxiangqing = this.ruleForm.fafangxiangqing.replace(reg,'../../../GZIM_SYS/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `fafangbutie/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.fafangbutieCrossAddOrUpdateFlag = false;
											this.parent.search();
