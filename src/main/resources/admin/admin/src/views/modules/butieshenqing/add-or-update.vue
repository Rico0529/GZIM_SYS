	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="upload" v-if="type!='info'&& !ro.xiangmuziliao" label="项目资料" prop="xiangmuziliao" >
					<file-upload
						tip="点击上传项目资料"
						action="file/upload"
						:limit="1"
				...
				<el-form-item v-else-if="ruleForm.xiangmuziliao" label="项目资料" prop="xiangmuziliao" >
					<el-button class="downBtn" type="text" size="small" @click="download($base.url+ruleForm.xiangmuziliao)">
				...
				<el-form-item class="input" v-if="type!='info'"  label="申请资金" prop="shenqingzijin" >
					<el-input-number v-model="ruleForm.shenqingzijin" placeholder="申请资金" :disabled="ro.shenqingzijin"></el-input-number>
				</el-form-item>
				<el-form-item v-else class="input" label="申请资金" prop="shenqingzijin" >
					<el-input v-model="ruleForm.shenqingzijin" placeholder="申请资金" readonly></el-input>
				</el-form-item>
	...
	export default {
		...
		methods: {
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='shenqingzijin'){
							this.ruleForm.shenqingzijin = obj[o];
							this.ro.shenqingzijin = true;
							continue;
						}
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.shenqingxiangqing = this.ruleForm.shenqingxiangqing.replace(reg,'../../../GZIM_SYS/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					if(this.ruleForm.xiangmuziliao!=null) {
						this.ruleForm.xiangmuziliao = this.ruleForm.xiangmuziliao.replace(new RegExp(this.$base.url,"g"),"");
					}
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `butieshenqing/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.butieshenqingCrossAddOrUpdateFlag = false;
											this.parent.search();
			...
			xiangmuziliaoUploadChange(fileUrls) {
				this.ruleForm.xiangmuziliao = fileUrls;
			},
		}
	};
</script>
