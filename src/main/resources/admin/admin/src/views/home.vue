<template>
	<div class="home-content">
		...
		<div class="statis-box">
			...
			<div id="statis2" class="statis2 animate__animated" v-if="isAuth('shebaojiaofei','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-chujia3"></span>
				</div>
				<div class="right">
					<div class="num">{{shebaojiaofeiCount}}</div>
					<div class="name">社保缴费总数</div>
				</div>
			</div>
			<div id="statis3" class="statis3 animate__animated" v-if="isAuth('toudijianli','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-qita3"></span>
				</div>
				<div class="right">
					<div class="num">{{toudijianliCount}}</div>
					<div class="name">投递简历总数</div>
				</div>
			</div>
		</div>
		<!-- statis -->
		...
		<div class="type4">
			...
			<div id="yonghuChart2" class="echarts2 animate__animated" v-if="isAuth('yonghu','首页统计')"></div>
			<div id="shebaojiaofeiChart1" class="echarts3 animate__animated" v-if="isAuth('shebaojiaofei','首页统计')"></div>
			<div id="toudijianliChart1" class="echarts4 animate__animated" v-if="isAuth('toudijianli','首页统计')"></div>
		</div>
	</div>
</template>
<script>
...
export default {
	...
	mounted(){
		...
		this.getshebaojiaofeiCount();
		if(this.isAuth('shebaojiaofei','首页统计')){
			this.shebaojiaofeiChat1();
		}
		this.gettoudijianliCount();
		if(this.isAuth('toudijianli','首页统计')){
			this.toudijianliChat1();
		}
	...
	methods:{
		handleScroll() {
			let arr = [
				...
				{id:'yonghuChart2',css:'animate__fadeInUp'},
				{id:'shebaojiaofeiChart1',css:'animate__fadeInUp'},
				{id:'toudijianliChart1',css:'animate__fadeInUp'},
			]
...
// 1234 折
		...
		getshebaojiaofeiCount() {
			this.$http({
				url: `shebaojiaofei/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.shebaojiaofeiCount = data.data
				}
			})
		},
// 1234 饼2
		shebaojiaofeiChat1() {
			this.$nextTick(()=>{
				...
				var shebaojiaofeiChart1 = echarts.init(document.getElementById("shebaojiaofeiChart1"),'macarons');
				this.$http({
					url: `shebaojiaofei/value/yonghuzhanghao/shebaojiage`,
					method: "get",
				}).then(({ data }) => {
					if (data && data.code === 0) {
						let res = data.data;
						...
						// 使用刚指定的配置项和数据显示图表。
						shebaojiaofeiChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							shebaojiaofeiChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		gettoudijianliCount() {
			this.$http({
				url: `toudijianli/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.toudijianliCount = data.data
				}
			})
		},
// 1234 竖
		toudijianliChat1() {
			this.$nextTick(()=>{
				...
				var toudijianliChart1 = echarts.init(document.getElementById("toudijianliChart1"),'macarons');
				this.$http({
					url: `toudijianli/value/zhaopinmingcheng/gangweixinzi`,
					method: "get",
				}).then(({ data }) => {
					if (data && data.code === 0) {
						let res = data.data;
						...
						for(let i=0;i<res.length;i++){
							...
							xAxis.push(res[i].zhaopinmingcheng);
							yAxis.push(parseFloat((res[i].total)));
							pArray.push({
								value: parseFloat((res[i].total)),
								name: res[i].zhaopinmingcheng
							})
						}
						var option = {};
						let titleObj = this.bar.title
						...
						// 使用刚指定的配置项和数据显示图表。
						toudijianliChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							toudijianliChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
